...
import { ThemeIcon } from "@/components/package-manager-install-toolbar";
import { useCallback, useEffect, useState } from "react";

type ThemeMode = "light" | "dark";

const THEME_STORAGE_KEY = "lwc-theme";
const DEFAULT_THEME: ThemeMode = "dark";
const THEME_TRANSITION_STYLE_ID = "lwc-theme-transition-styles";
...
function createThemeTransitionCss() {
  return `
    ...
    ::view-transition-new(root) {
      animation-name: lwc-theme-reveal;
    }

    @keyframes lwc-theme-reveal {
      from {
  ...
  return (
    ...
    >
      <span className="flex min-w-0 flex-1 items-center justify-center gap-1 rounded-lg bg-bg p-[7px]">
        <ThemeIcon className="size-4 sm:size-5" />
      </span>
    </button>
  );
}
