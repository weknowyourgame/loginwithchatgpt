...
/** Match CLI / Manual dot rail in the install card. */
...
/** Euclidean ring from grid center (cell 5,5) — matches `DotIcon` ripple staggering. */
...
const HEART_DOTS: ReadonlyArray<readonly [number, number]> = [
  [8, 4],
  [10, 4],
  [14, 4],
  [16, 4],
...
export function CopyClipboardIcon({ className, size = 18 }: DotIconProps) {
...
export function CheckIcon({ className, size = 18 }: DotIconProps) {
  const activeDots = new Set<string>([
    // Clipboard shell
    dotKey(10, 2), dotKey(12, 2), dotKey(14, 2),
    dotKey(8, 4), dotKey(10, 4), dotKey(12, 4), dotKey(14, 4), dotKey(16, 4),
    ...
    dotKey(20, 6), dotKey(20, 8), dotKey(20, 10), dotKey(20, 12), dotKey(20, 14), dotKey(20, 16), dotKey(20, 18), dotKey(20, 20),
    dotKey(6, 22), dotKey(8, 22), dotKey(10, 22), dotKey(12, 22), dotKey(14, 22), dotKey(16, 22), dotKey(18, 22),
    // Inner success pattern (dot version of provided points)
...
export function ThemeIcon({ className, size = 18 }: DotIconProps) {
  const activeDots = new Set<string>([
...
/** Home silhouette icon on a 24x24 dot grid. */
export function HomeIcon({ className, size = 18 }: DotIconProps) {
  const activeDots = new Set<string>([
    dotKey(16, 2),
    dotKey(16, 4),
    dotKey(12, 4),
...
export function HeartIcon({ className, size = 18 }: DotIconProps) {
...
function CodeBlockWithCopy({
  ...
  // Only branch on reduced-motion after mount so SSR and first client render match
  // (useReducedMotion resolves to a real value only on the client → hydration mismatch otherwise).
